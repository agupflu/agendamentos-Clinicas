...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (r.ok) { router.push("/inicio"); router.refresh(); }
      else { const d = await r.json(); setErro(d.error ?? "Credenciais inválidas."); }
    ...
    <div style={{ minHeight: "100vh", background: "#080808", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px" }}>
      <div style={{ width: "100%", maxWidth: "380px" }}>
        <div style={{ textAlign: "center", marginBottom: "36px" }}>
          ...
          <h1 style={{ fontSize: "22px", fontWeight: "700", color: "#fff", marginBottom: "6px" }}>Clínica Sistema</h1>
        ...
        <form onSubmit={handleLogin} style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
          <div>
            ...
            <div style={{ position: "relative" }}>
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required
                ...
                style={{ width: "100%", padding: "12px 14px 12px 36px", background: "#111", border: `1px solid ${BORDER}`, borderRadius: "8px", color: "#fff", fontSize: "14px", outline: "none" }} />
            </div>
          </div>
          ...
          {erro && <p style={{ fontSize: "13px", color: "#EF4444", padding: "10px 14px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: "8px" }}>{erro}</p>}
